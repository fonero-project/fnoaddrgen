...
import (
	...
	"github.com/fonero-project/fnowallet/walletseed"
)
...
func generateKeyPair(filename string) error {
	...
	addr, err := fnoutil.NewAddressPubKeyHash(
		fnoutil.Hash160(pub.SerializeCompressed()),
		&params,
	...
	privWif, err := fnoutil.NewWIF(priv, &params, fnoec.STEcdsaSecp256k1)
	if err != nil {
		return err
	}
...
func main() {
	...
	helpMessage := func() {
		fmt.Println(
			"Usage: fnoaddrgen [-testnet] [-simnet] [-noseed] [-verify] [-h] filename")
		fmt.Println("Generate a Fonero private and public key or wallet seed. \n" +
